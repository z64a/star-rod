...
uniform sampler2D u_image;	// the framebuffer
uniform vec4 u_viewport;	// relative to full frame buffer; ie, within [0,1]
uniform vec2 u_resolution;	// full size of frame buffer, in pixels
...
vec4 sampleBuffer(vec2 uv, float bias)
{
	return texture(u_image, vp2fb(uv), bias);
}
...
vec3 Fetch(vec2 pos, vec2 off)
{
	...
	if(max(abs(pos.x - 0.5), abs(pos.y - 0.5)) > 0.5)
		...
		return ToLinear(sampleBuffer(pos.xy, -16.0).rgb);
}
...
void main()
{
	vec2 pos = Warp(fb2vp(f_texCoords));
	o_color.rgb = Bloom(pos)*Mask(f_texCoords * u_resolution);
	o_color.rgb = ToSrgb(o_color.rgb);
	o_color.a = 1.0;
}
