...
public class HemisphereGenerator extends ShapeGenerator
{
	...
	public HemisphereGenerator()
	{
		super(Primitive.HEMISPHERE);

		radiusSpinner = new LabeledIntegerSpinner("Radius", 10, 5000, 200);
		latFacesSpinner = new LabeledIntegerSpinner("Vertical Segments", 3, 32, 6);
	...
	private static TriangleBatch generate(int radius, int latfaces, int longfaces, int centerX, int centerY, int centerZ)
	{
		...
		for (int j = 0; j <= longfaces; j++) {
			...
			rings[latfaces][j] = new Vertex(centerX, centerY, centerZ);
		...
		for (int i = 1; i <= latfaces; i++)
			for (int j = 0; j <= longfaces; j++) {
				...
				double theta = (Math.PI * i / (2 * latfaces));
